...
/**
 * Returns the socket resourcs if the connection succeds.
 *
 * @param $host
 * @param $port
 *
    ...
    if ($result = @socket_write($socket, $packet, strlen($packet)))
    {
        if ($buf = @socket_read($socket, 2, PHP_BINARY_READ)) // login response packet is 2 bytes
        {
            ...
            if ($bytes[1] == 0x02) // login response header 0x02
            {
                ...
                {
                    case 0x04:
                        socket_read($socket, 1024, PHP_BINARY_READ); // read away the client info data in the stream from RemoteAdmin
                        return true;
                        break;
...
function world_save($socket)
{
    ...
    $packet = "\xf1\x00" . chr($packet_size) . "\x41";
